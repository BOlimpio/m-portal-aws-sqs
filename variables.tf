...
variable "dlq_visibility_timeout_seconds" {
  description = "The duration (in seconds) that the received messages are hidden from the Dead Letter Queue (DLQ)."
  type        = number
  default     = 30
}

variable "dlq_message_retention_seconds" {
  description = "The number of seconds that Amazon SQS retains a message in the Dead Letter Queue (DLQ)."
  type        = number
  default     = 345600 # 4 days
}

variable "dlq_max_message_size" {
  description = "The limit of how many bytes a message can contain before Amazon SQS rejects it in the Dead Letter Queue (DLQ)."
  type        = number
  default     = 262144 # 256 KB
}

variable "dlq_delay_seconds" {
  description = "The time in seconds that the delivery of all messages in the Dead Letter Queue (DLQ) is delayed."
  type        = number
  default     = 0
}

variable "dlq_receive_wait_time_seconds" {
  description = "The maximum wait time for receiving a message from the Dead Letter Queue (DLQ)."
  type        = number
  default     = 0
}
...
variable "dlq_fifo_queue" {
  description = "Boolean designating a FIFO queue for the Dead Letter Queue (DLQ)."
  type        = bool
  default     = false
}

variable "dlq_content_based_deduplication" {
  description = "Boolean designating content-based deduplication for the Dead Letter Queue (DLQ)."
  type        = bool
  default     = false
}

variable "dlq_sqs_managed_sse_enabled" {
  description = "Boolean designating whether to enable SQS managed server-side encryption for the Dead Letter Queue (DLQ)."
  type        = bool
  default     = false
}

variable "dlq_kms_master_key_id" {
  description = "The ID of an AWS managed customer master key (CMK) for Amazon SQS or a custom CMK for the Dead Letter Queue (DLQ)."
  type        = string
  default     = null
}

variable "dlq_kms_data_key_reuse_period_seconds" {
  description = "The length of time, in seconds, for which Amazon SQS can reuse a data key to encrypt or decrypt messages in the Dead Letter Queue (DLQ) before calling AWS KMS again."
  type        = number
  default     = null
}
