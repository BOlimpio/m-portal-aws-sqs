...
data "aws_iam_policy_document" "sqs_policy" {
  source_policy_documents = concat(
    ...
    [
      length(var.producer_principals) > 0 ? data.aws_iam_policy_document.producer_policy.json : null,
      length(var.consumer_principals) > 0 ? data.aws_iam_policy_document.consumer_policy.json : null,
      length(var.admin_principals) > 0 ? data.aws_iam_policy_document.admin_policy.json : null,
    ]
  )
}
...
data "aws_iam_policy_document" "dlq_policy" {
  source_policy_documents = concat(
    ...
    [
      length(var.dlq_producer_principals) > 0 ? data.aws_iam_policy_document.dlq_producer_policy.json : null,
      length(var.dlq_consumer_principals) > 0 ? data.aws_iam_policy_document.dlq_consumer_policy.json : null,
      length(var.dlq_admin_principals) > 0 ? data.aws_iam_policy_document.dlq_admin_policy.json : null,
    ]
  )
}
